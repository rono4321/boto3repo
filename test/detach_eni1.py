...
for each_nic in response['NetworkInterfaces']:
  ...
  if desc != None and eni_name in desc:
    #print("Found!")
    eni_eni_this=each_nic['NetworkInterfaceId']
    eni_attach_this=each_nic['Attachment']['AttachmentId']
    response1 = ec2.detach_network_interface(AttachmentId=eni_attach_this,Force=True|False)
    try:
      ...
      print("Ok")
    time.sleep(20)
    response3 = ec2.delete_network_interface(NetworkInterfaceId=eni_eni_this)
